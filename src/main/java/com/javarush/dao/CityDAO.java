...
public class CityDAO {private final SessionFactory sessionFactory;
    ...
    public CityEntity getById(Integer id) {
        Query<CityEntity> query = sessionFactory.getCurrentSession().createQuery("select c from CityEntity c join fetch c.country where c.id = :ID", CityEntity.class);
